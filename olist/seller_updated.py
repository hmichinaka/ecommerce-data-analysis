...
class Seller:
    ...
    def get_active_dates(self):
        ...
        orders_sellers = orders_approved.merge(self.data['order_items'],
                        on='order_id')[['order_id', 'seller_id','order_approved_at']]\
                        .drop_duplicates()
        ...
        df = orders_sellers.groupby('seller_id').agg({"date_first_sale": min,
                                                        "date_last_sale": max})
        df['months_on_olist'] = round((df['date_last_sale'] - df['date_first_sale']) /
            np.timedelta64(1, 'M'))
        return df
    ...
    def get_sales(self):
        ...
        return self.data['order_items'][['seller_id', 'price']]\
            .groupby('seller_id').sum().rename(columns={'price': 'sales'})
    ...
    def get_training_data(self):
        ...
        training_set['profits'] = training_set['revenues'] - training_set['review_costs']
